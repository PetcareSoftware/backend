...
from .models import (
    Supplier, Supply, SupplyBatch, ConsultationSupply,
    PurchaseOrder, PurchaseOrderItem, ClinicalProcedureSupply,
    AuditSupplyBatch,
)
...
class SupplyBatchAdmin(admin.ModelAdmin):
    list_display = ('supply', 'lot_number', 'expiration_date', 'current_stock', 'acquisition_cost')
...
class ClinicalProcedureSupplyAdmin(admin.ModelAdmin):
    ...
    search_fields = ('procedure_id',)


@admin.register(AuditSupplyBatch)
class AuditSupplyBatchAdmin(admin.ModelAdmin):
    list_display = ('audit_id', 'batch_id', 'action', 'changed_at', 'changed_by')
    list_filter = ('action', 'changed_at')
    readonly_fields = ('audit_id', 'batch_id', 'action', 'old_data', 'new_data', 'changed_at', 'changed_by')